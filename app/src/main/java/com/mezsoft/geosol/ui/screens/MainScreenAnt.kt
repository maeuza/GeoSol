...
@Composable
fun MainScreen(mainViewModel: MainViewModelAnt = hiltViewModel()
    ...
    var resultScan: Pair<String, String>
    var lote = 0
    var palma = 0
    var down = " "
    val showButton: Boolean by mainViewModel.showButton.observeAsState(false)
    val okDownload: Boolean by mainViewModel.okDownload.observeAsState(false)
    val locGranted: Boolean by mainViewModel.okDownload.observeAsState(false)
    ...
    if (currentYear<2025 && currentMount<2) {
       ...
       CurrentLocationScreen(mainViewModel)
        ...
        if (locGranted) {
            Column(
                ...
                Box(
                    ...
                    Button(
                        onClick = { mainViewModel.startScanning() },
                        modifier = Modifier
                            .width(260.dp)
                            .height(100.dp)
                    ) {
                ...
                Box(
                    ...
                    Row(horizontalArrangement = Arrangement.SpaceBetween) {
                        ...
                        if (showButton) {
                            Button(
                                onClick = { mainViewModel.onDownload() },
                        ...
                        if (!showButton) {
                            if (state.value.details.isNotEmpty()) {
                                Button(
                                    onClick = {
                                        mainViewModel.onShowButtonClick()
            ...
            if (state.value.details.isNotEmpty()) {
                ...
                mainViewModel.addLotePalma(plantModel)
            }
        }
    }
}
